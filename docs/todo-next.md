...
- Continue acting on the 2026-04-30 external audit triage after the first runtime Risk/OMS gate, durable recovery, broker-source MT5 hedging, deal-based accounting, durable per-deal attribution, protective SL/TP, position-protection fail-safe, risk-config guard, MT5 symbol-spec quantization/enforcement, MT5 reconnect supervision, approved protection-flatten, startup recovery/fault-injection, MT5 protective repair/modify, post-repair kill-switch reset, pending-order modify, history-probe tooling, and runtime supervisor slices: next validate the new history probe against the real Parallels MT5 demo terminal, continue alert routing/operator automation, Docker validation on a Docker-enabled host, and small-batch Ruff/mypy cleanup.
- Keep the completed Parallels MT5 demo-order findings in scope while doing this work: Dukascopy `margin_mode=2` behaved as hedging, IOC is required for EURUSD, and previous `history_orders_get` / `history_deals_get` calls still returned zero rows after controlled demo fills.
