...
| Execution | `tests/unit/test_execution_*.py`, `tests/unit/test_scripts_mt5_broker_probe.py`, `tests/unit/test_scripts_mt5_demo_order.py`, `tests/integration/test_execution_workflow.py` | paper adapter, router, live stub, MT5 adapter/client fakes, reconciliation, safe broker probe payloads, controlled demo-order safety gates | no real broker; fake MT5 modules | passed locally |
...
- MT5 unit tests use fake modules, while the Windows/Parallels broker probes now prove connection, symbol/tick polling, empty history polling, broker-specific filling-mode handling, and 29-character comment clamping against real authorized demo terminals without `order_send`.
...
- Linting is exposed through `make lint` and `make lint-baseline`. The Python 3.12.13 `.venv` baseline currently reports `374` historical Ruff issues after the scripts, config-layer, logging-utils, journal, OMS, validation, models, and risk cleanup batches; newly touched code should keep targeted Ruff checks green.
