...
- Linting is exposed through `make lint` and `make lint-baseline`. The Python 3.12.13 `.venv` baseline currently reports `433` historical Ruff issues after the scripts, config-layer, and logging-utils cleanup batches; newly touched code should keep targeted Ruff checks green.
