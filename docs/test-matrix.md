...
- Linting is exposed through `make lint` and `make lint-baseline`. The Python 3.12.13 `.venv` baseline currently reports `496` historical Ruff issues after the first scripts cleanup batch; newly touched code should keep targeted Ruff checks green.
