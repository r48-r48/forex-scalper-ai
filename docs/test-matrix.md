...
| Validation | `tests/unit/test_validation_metrics.py`, `tests/unit/test_validation_baseline_suite.py`, `tests/integration/test_validation_walk_forward.py`, `tests/integration/test_baseline_walk_forward_suite.py` | fold metrics, walk-forward orchestration, backtest-frame conversion, baseline suite and sensitivity reports | pandas | passed locally |
...
| Python 3.11+ full suite | passing | Python 3.12.13 `.venv`, `148 passed` |
...
- Linting is exposed through `make lint`. The local Python 3.9.6 environment does not include ruff; the Python 3.12.13 `.venv` can run ruff, but full-repo lint still has a historical backlog while the new P1.1/P1.2 files pass targeted ruff checks.
