...
- continue real MT5 history/deal investigation: Dukascopy demo fills closed cleanly, but `history_orders_get` / `history_deals_get` still returned zero rows after controlled fills; keep IOC execution settings for Dukascopy EURUSD unless broker metadata changes
