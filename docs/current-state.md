...
- The next work should continue from the completed runtime Risk/OMS gate, durable recovery, broker-source MT5 hedging, deal-accounting/per-deal attribution slices, protective-order slices, position-protection fail-safe, first risk-config guard slice, first MT5 symbol-spec quantization slice, first MT5 reconnect supervision slice, and first approved protection-flatten workflow into deeper broker-side startup recovery, non-empty history/deal normalization on a real broker, fuller symbol metadata enforcement for prices/stops/filling, long-running daemon supervision, Docker runtime validation on a Docker-enabled host, and retiring the full-repo lint/typecheck baseline in small batches.
...
- add deeper broker-side startup recovery and fault-injection tests for broker-only positions, partial local state, reconnect drift, and missing/empty broker history
...
- extend symbol-specific metadata enforcement to price precision, stop/freeze levels, trade mode, and filling mode; if MT5 validation is paused, validate Docker/Compose runtime packaging on a Docker-enabled host, then continue with network alert transport and small-batch Ruff/mypy cleanup from `docs/post-phase-roadmap.md`
