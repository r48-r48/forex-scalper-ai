...
- The next work should continue from the completed runtime Risk/OMS gate, durable recovery, broker-source MT5 hedging, first deal-accounting slice, and first protective-order slice into richer bracket/position-protection workflows, deeper broker-side startup recovery, non-empty history/deal normalization on a real broker, symbol-specific quantization, Docker runtime validation on a Docker-enabled host, and retiring the full-repo lint/typecheck baseline in small batches.
