...
- continue real MT5 history/deal investigation with the new read-only `scripts/mt5_history_probe.py`: Dukascopy demo fills closed cleanly, but previous `history_orders_get` / `history_deals_get` windows returned zero rows after controlled fills; keep IOC execution settings for Dukascopy EURUSD unless broker metadata changes
