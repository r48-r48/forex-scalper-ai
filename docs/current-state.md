...
- The next work should continue from the completed runtime Risk/OMS gate, first durable recovery slice, and first broker-source MT5 hedging slice into deal-based accounting, protective order management, deeper broker-side startup recovery, non-empty history/deal normalization, symbol-specific quantization, Docker runtime validation on a Docker-enabled host, and retiring the full-repo lint/typecheck baseline in small batches.
