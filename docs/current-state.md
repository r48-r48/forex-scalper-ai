...
- The next work should continue from the completed runtime Risk/OMS submit gate into durable state/recovery, broker-source-of-truth MT5 sizing, hedging-aware execution/reconciliation, deal-based accounting, protective order management, non-empty history/deal normalization, Docker runtime validation on a Docker-enabled host, and retiring the full-repo lint/typecheck baseline in small batches.
