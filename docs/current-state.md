...
- The next work should continue from the completed runtime Risk/OMS gate, durable recovery, broker-source MT5 hedging, first deal-accounting slice, first protective-order slice, first risk-config guard slice, and first MT5 symbol-spec quantization slice into richer bracket/position-protection workflows, deeper broker-side startup recovery, non-empty history/deal normalization on a real broker, fuller symbol metadata enforcement for prices/stops/filling, MT5 reconnect supervision, Docker runtime validation on a Docker-enabled host, and retiring the full-repo lint/typecheck baseline in small batches.
