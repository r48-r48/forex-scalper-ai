...
2. The production training/model bundle pipeline is still incomplete:
   supervised baseline and transformer training/export CLIs plus runtime inference packages now exist, but a model registry, drift monitoring, and end-to-end live signal loop are still pending.
...
- Completed on `2026-05-03`: transformer training/export and runtime inference packaging were added with `scripts/train_transformer.py`, `model.pt`/`scaler.json`/`metadata.json` bundle export, SHA-verified `load_transformer_inference_package()`, tail validation split reporting, and tests for leakage-safe cutoff behavior plus runtime scoring; `make PYTHON=.venv/bin/python ci` passed with full pytest `334 passed`.
