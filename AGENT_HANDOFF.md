...
- Full-repo Ruff/mypy cleanup baseline completed on 2026-04-28 as `docs/lint-typecheck-baseline.md`: Ruff initially had `511` historical issues and mypy had `51` errors in `30` files; both are now fully green and wired into the local/CI gate
...
- Data-layer Ruff cleanup completed on 2026-05-03: full Ruff baseline dropped from `374` to `163` issues
...
- Data-layer Ruff cleanup passed on 2026-05-03: targeted Ruff is green for `src/scalper_ai/data` plus selected data unit/integration tests; full Ruff baseline dropped from `374` to `163` issues
...
- Backtesting Ruff cleanup passed on 2026-05-03: targeted Ruff is green for `src/scalper_ai/backtesting` plus selected backtesting tests; full Ruff baseline dropped from `163` to `138` issues
...
- Features Ruff cleanup passed on 2026-05-03: targeted Ruff is green for `src/scalper_ai/features` plus selected feature tests; full Ruff baseline dropped from `138` to `93` issues
...
- Last completed post-phase hardening milestone:
  ...
  - fresh-checkout Parallels MT5 validation is complete: cloned current `main` into `C:\Users\dzhabrailtalkanov\projects\forex-scalper-ai-current` inside Windows 11, verified commit `86afc80` with clean `main...origin/main` state, reused the existing Windows `.venv` Python to run fresh scripts, read-only smoke connected to Dukascopy demo account `610769553` with zero open orders/positions, safe EURUSD IOC broker probe accepted `order_check` with `retcode=0` / `Done`, raw 8760-hour history returned 6 orders / 5 deals including the known zero-volume deposit shape, and `order_send_called=false`
  - external audit 2026-05-03 live-safety triage is complete: `/Users/dzhabrailtalkanov/Downloads/deep-research-report (2).md` was reviewed and persisted as `docs/external-audit-2026-05-03.md`; confirmed fixes were applied so `configs/live.yaml` uses the implemented MT5 adapter with paper fallback disabled and MT5 stop-loss required, live startup fails closed without data/model/guard providers, and runtime daily drawdown uses a separate UTC day-start equity baseline; `make PYTHON=.venv/bin/python ci` passed with full pytest `270 passed`
  - external audit 2026-05-03 parallel follow-ups are complete: added `scripts/create_release_archive.py` for clean source exports, `src/scalper_ai/data/quality.py` for offline tick/replay QA reports, and `src/scalper_ai/models/bundle.py` for model bundle metadata with deterministic feature-contract hashing; targeted Ruff/compile/tests passed with `20 passed`, and `make PYTHON=.venv/bin/python ci` passed with full pytest `290 passed`
  - data-layer Ruff cleanup is complete: `src/scalper_ai/data` and selected data tests now pass targeted Ruff; selected data pytest passed with `20 passed`; compileall, `git diff --check`, and full pytest passed with `265 passed`; full Ruff backlog dropped from `374` to `163`
  ...
  - Wall-clock local paper supervision evidence is complete: `make supervise-paper-wallclock` persists bounded JSON evidence, and the first 2-second run produced 11 pass iterations with zero alerts/errors
  - Fresh-checkout Parallels MT5 validation is complete: use `C:\Users\dzhabrailtalkanov\projects\forex-scalper-ai-current` for future latest-code checks; current `main` commit `86afc80` smoke/probe passed with explicit `BROKER_MT5_TERMINAL_PATH`, zero open orders/positions, IOC `order_check` accepted, raw history visible, and no `order_send`
...
- The exact next task is post-phase hardening:
  ...
  - FX backtest realism now covers row-level costs, pip value, margin-rate/swap metrics, margin-level/effective-leverage metrics, opt-in margin-call liquidation, and prior-row stop-loss/take-profit path simulation; continue with broker symbol-spec ingestion and deeper execution-path calibration
...
- keep Docker/Compose runtime packaging, Ruff, mypy, compileall, pytest, safe MT5 preflight, and paper supervisor evidence green while continuing dependency supervision, longer paper/shadow evidence with real alert sink topology, and remaining MT5 fault-injection checks
